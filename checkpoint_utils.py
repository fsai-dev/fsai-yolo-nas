...
def maybe_remove_module_prefix(
    state_dict: Mapping[str, Tensor], prefix: str = "module."
) -> Mapping[str, Tensor]:
    """
    ...
    if all([key.startswith(prefix) for key in state_dict.keys()]):
        state_dict = collections.OrderedDict(
            [(key[offset:], value) for key, value in state_dict.items()]
        )
    return state_dict


def adaptive_load_state_dict(
    net: torch.nn.Module, state_dict: dict, strict: Union[bool, StrictLoad], solver=None
):
    """
    ...
    try:
        ...
        if strict == StrictLoad.NO_KEY_MATCHING:
            adapted_state_dict = adapt_state_dict_to_fit_model_layer_names(
                net.state_dict(), state_dict, solver=solver
            )
...
def copy_ckpt_to_local_folder(
    ...
    remote_ckpt_source_dir = (
        local_ckpt_destination_dir
        if remote_ckpt_source_dir is None
        else remote_ckpt_source_dir
    )
    ...
    if not overwrite_local_ckpt:
        ...
        download_ckpt_destination_dir = pkg_resources.resource_filename(
            "checkpoints", local_ckpt_destination_dir
        )

    if path_src.startswith("s3"):
        model_checkpoints_data_interface = ADNNModelRepositoryDataInterfaces(
            data_connection_location=path_src
        )
        # DOWNLOAD THE FILE FROM S3 TO THE DESTINATION FOLDER
        ckpt_file_full_local_path = (
            model_checkpoints_data_interface.load_remote_checkpoints_file(
        ...
        if not load_weights_only:
            ...
            model_checkpoints_data_interface.load_all_remote_log_files(
                model_name=remote_ckpt_source_dir,
                model_checkpoint_local_dir=download_ckpt_destination_dir,
            )

    if path_src == "url":
        ckpt_file_full_local_path = (
            download_ckpt_destination_dir + os.path.sep + ckpt_filename
        )
        # DOWNLOAD THE FILE FROM URL TO THE DESTINATION FOLDER
        with wait_for_the_master(get_local_rank()):
            download_url_to_file(
                remote_ckpt_source_dir, ckpt_file_full_local_path, progress=True
            )
    ...
    if ckpt_path.startswith("https://"):
        with wait_for_the_master(get_local_rank()):
            state_dict = load_state_dict_from_url(
                ckpt_path, progress=False, map_location=device
            )
        return state_dict
    else:
        if not os.path.exists(ckpt_path):
            raise FileNotFoundError(
                f"Incorrect Checkpoint path: {ckpt_path} (This should be an absolute path)"
            )
...
class DefaultCheckpointSolver:
    ...
    def __call__(
        ...
        """
        new_ckpt_dict = {}
        for (ckpt_key, ckpt_val), (model_key, model_val) in zip(
            checkpoint_state_dict.items(), model_state_dict.items()
        ):
            if ckpt_val.shape != model_val.shape:
                raise ValueError(
                    f"ckpt layer {ckpt_key} with shape {ckpt_val.shape} does not match {model_key}"
...
class YoloXCheckpointSolver:
    ...
    def generate_mapping_table(cls) -> Mapping[str, str]:
        ...
        for model_name in model_names:
            ...
            for (model_key, model_value), (checkpoint_key, checkpoint_value) in zip(
                model_state_dict.items(), new_sd.items()
            ):
                if (
                    model_value.size() == checkpoint_value.size()
                    and model_key.split(".")[-1] == checkpoint_key.split(".")[-1]
                ):
    ...
    def _remove_saved_stride_tensors(self, state_dict):
        ...
        return collections.OrderedDict(
            [(k, v) for k, v in state_dict.items() if k not in exclude_stride_keys]
        )
    ...
    def _yolox_ckpt_solver(self, ckpt_key, ckpt_val, model_key, model_val):
        ...
        if (
            ...
            and (
                ckpt_key == "module._backbone._modules_list.0.conv.conv.weight"
                or ckpt_key == "_backbone._modules_list.0.conv.conv.weight"
            )
...
def adapt_state_dict_to_fit_model_layer_names(
...
def raise_informative_runtime_error(state_dict, checkpoint, exception_msg):
    ...
    """
    try:
        new_ckpt_dict = adapt_state_dict_to_fit_model_layer_names(
        ...
        ValueError
    ) as ex:  # IN CASE adapt_state_dict_to_fit_model_layer_names WAS UNSUCCESSFUL
...
def load_checkpoint_to_model(
    ...
    if load_ema_as_net:
        if "ema_net" not in checkpoint.keys():
            raise ValueError(
                "Can't load ema network- no EMA network stored in checkpoint file"
            )
        else:
...
def load_pretrained_weights(
    model: torch.nn.Module, architecture: str, pretrained_weights: str
):
    """
    ...
    if url.startswith("file://") or os.path.exists(url):
        ...
        unique_filename = (
            url.split("https://sg-hub-nv.s3.amazonaws.com/models/")[1]
            .replace("/", "_")
            .replace(" ", "_")
        )
...
def load_pretrained_weights_local(
    model: torch.nn.Module, architecture: str, pretrained_weights: str
):
    """
    Loads pretrained weights from the MODEL_URLS dictionary to model
...
def _load_weights(architecture, model, pretrained_state_dict):
    ...
    solver = (
        ...
        if "yolox" in architecture
        else DefaultCheckpointSolver()
    )
    ...
    logger.info(
        f"Successfully loaded pretrained weights for architecture {architecture}"
    )


def _maybe_load_preprocessing_params(
    model: Union[nn.Module, HasPredict], checkpoint: Mapping[str, Tensor]
) -> bool:
    """
    ...
    if model_has_predict and checkpoint_has_preprocessing_params:
        try:
            ...
            logger.debug(
                f"Successfully loaded preprocessing params from checkpoint {checkpoint['processing_params']}"
            )
            return True
