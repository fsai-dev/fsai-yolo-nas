...
def reduce_results_tuple_for_ddp(validation_results_tuple, device):
    ...
    for i, validation_result in enumerate(validation_results_list):
        ...
        validation_results_list[i] = distributed_all_reduce_tensor_average(
            tensor=validation_result.to(device), n=torch.distributed.get_world_size()
        )
...
def compute_precise_bn_stats(
    ...
    num_iter = (
        int(precise_bn_batch_size / (loader.batch_size * num_gpus))
        if precise_bn_batch_size
        else num_gpus
    )
...
def setup_device(
    multi_gpu: MultiGPUMode = None, num_gpus: int = None, device: str = "cuda"
):
    """
    ...
    if device == "cpu":
        ...
        raise ValueError(
            f"Only valid values for device are: 'cpu' and 'cuda'. Received: '{device}'"
        )
...
def setup_cpu(multi_gpu: MultiGPUMode = MultiGPUMode.AUTO, num_gpus: int = None):
    ...
    if multi_gpu not in (MultiGPUMode.OFF, MultiGPUMode.AUTO, None):
        raise ValueError(
            f"device='cpu' and multi_gpu={multi_gpu} are not compatible together."
        )

    if num_gpus not in (0, None):
        raise ValueError(
            f"device='cpu' and num_gpus={num_gpus} are not compatible together."
        )
...
def _resolve_gpu_params(
    multi_gpu: MultiGPUMode, num_gpus: int
) -> Tuple[MultiGPUMode, int]:
    """
    ...
    if multi_gpu == MultiGPUMode.OFF:
        if num_gpus != 1:
            raise ValueError(
                f"You specified num_gpus={num_gpus} but it has not be 1 on when working with multi_gpu={multi_gpu}"
            )
    else:
        if num_gpus > torch.cuda.device_count():
            raise ValueError(
                f"You specified num_gpus={num_gpus} but only {torch.cuda.device_count()} GPU's are available"
            )
...
def initialize_ddp():
    ...
    if torch.distributed.get_rank() == 0:
        logger.info(
            f"Training in distributed mode... with {str(torch.distributed.get_world_size())} GPUs"
        )
...
def maybe_all_reduce_tensor_average(tensor: torch.Tensor) -> torch.Tensor:
    ...
    if is_distributed():
        ...
        tensor = distributed_all_reduce_tensor_average(
            tensor=tensor.to_dense(), n=torch.distributed.get_world_size()
        )
    return tensor
